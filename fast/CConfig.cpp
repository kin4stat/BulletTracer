...
// ќпредел€ем размер массива, чтобы в будущем не бегать по всему файлу и править при смене размера массива.
...
void CConfig::LoadSettings() {
	...
	// ≈сли произошла ошибка загрузки, выставл€ем стандартные значени€
	if (pReg->IsError()) {
		this->LoadDefaults();
	}
}
