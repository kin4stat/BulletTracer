...
private:
	...
	// Массив хранящий структуру трасера
	Tracer tracers[100];
...
public:
	...
	// Так как DoBulletImpact - функция класса, она имеет тип вызова __thiscall (еще бывают __fastcall, __stdcall (WINAPI), __cdecl (Обычные функции), __declspec (Спец функция с параметрами)
	...
	// __fastcall передает аргументы через регистры, что быстрее обычной передачи через стек. Но если использовать его вместо __thiscall, то у нас появляется бесполезный второй аргумент
